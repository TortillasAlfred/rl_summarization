...
class AZSumMCTS(pl.LightningModule):
    def __init__(self, dataset, reward, hparams):
        super(AZSumMCTS, self).__init__()
        ...
        self.lambda_loss = hparams.lambda_loss

        self.__build_model(hparams.hidden_dim)
        self.model = AZSummModel(hparams.hidden_dim, hparams.decoder_dim, self.dropout,)
    ...
    def mcts(self, sent_contents, doc_contents, states, valid_sentences):
        ...
        return (
            ...
            torch.stack([torch.stack([m_i[3] for m_i in m]) for m in mcts_pures]),
            torch.stack([torch.stack([m_i[4] for m_i in m]) for m in mcts_pures]),
        )
    ...
    def forward(self, batch, subset):
        ...
        if subset == "train":
            (
                ...
                mcts_vals,
                sampling_probs,
            ) = self.mcts(sent_contents, doc_contents, states, valid_sentences)

            for step_idxs, step_probs, step_policies, step_vals in zip(
                mcts_idxs.permute(1, 0),
                sampling_probs.permute(1, 0, 2),
                mcts_probs.permute(1, 0, 2),
                mcts_vals.permute(1, 0, 2, 3),
            ):
                _, mcts_rewards = self.environment.update(
                    step_idxs,
                    Categorical(probs=step_probs),
                    is_mcts=True,
            ...
            for _ in range(self.n_sents_per_summary):
                ...
                action_values_dist = (
                    F.softmax(action_values.mean(-1), dim=-1) * valid_sentences
                )

                sampling_dist = torch.distributions.Categorical(
                    policy_dist.probs + action_values_dist
                )

                _, greedy_idxs = sampling_dist.probs.max(dim=-1)
                states, greedy_rewards = self.environment.update(
                    greedy_idxs,
                    sampling_dist,
                    policy=policy_dist.probs,
                    q_vals=action_values,
                )
            ...
            action_dist, available_sents, q_values = self.model.produce_affinities(
    ...
    def training_step(self, batch, batch_idx):
        generated_rewards, greedy_rewards, policy_loss, value_loss = self.forward(
            batch, subset="train"
        )

        loss = self.lambda_loss * policy_loss + (1 - self.lambda_loss) * value_loss
    ...
    def from_config(dataset, reward, config):
        return AZSumMCTS(dataset, reward, config,)


class AZSummModel(torch.nn.Module):
    def __init__(self, hidden_dim, decoder_dim, dropout):
        ...
        self.q_value_head = torch.nn.Sequential(
            torch.nn.Linear(hidden_dim * 2 * 3, decoder_dim),
            torch.nn.Dropout(dropout),
            torch.nn.ReLU(),
    ...
    def produce_affinities(self, sent_contents, doc_contents, states, valid_sentences):
        ...
        contents = torch.cat([doc_contents, summ_contents, sent_contents], dim=-1)
        policy_values = self.decoding(contents)
        q_values = self.q_value_head(contents)
        ...
        policy_values = policy_values * valid_sentences
        q_values = q_values * valid_sentences.unsqueeze(-1).repeat_interleave(3, dim=-1)

        return Categorical(probs=policy_values), valid_sentences, q_values
