...
# class RLSumMCTSPure(pl.LightningModule):
#     def __init__(self, dataset, reward, hparams):
#         super(RLSumMCTSPure, self).__init__()
...
#                 action_dist, valid_sentences = self.model.produce_affinities(
#                     sent_contents, doc_contents, states, valid_sentences
#                 )
...
#             )
#             action_dist, available_sents = self.model.produce_affinities(
...
#         return RLSumMCTSPure(dataset, reward, config,)
...
#         output = self.decoder(contents).squeeze(-1)

#         return output / output.sum(-1, keepdim=True)
