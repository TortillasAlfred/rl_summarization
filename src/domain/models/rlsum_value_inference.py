...
class RLSumValueInference(pl.LightningModule):
    def __init__(self, dataset, reward, hparams):
        super(RLSumValueInference, self).__init__()
    ...
    def mcts_inference(self, sent_contents, doc_contents, states, valid_sentences):
        self.model.share_memory()
        mcts_idxs = self.pool.map(
            mcts.RLSumValueInferenceProcess(
                model=self.model,
                n_samples=self.n_mcts_samples,
                c_puct=self.c_puct,
                n_sents_per_summary=self.n_sents_per_summary,
            ),
            zip(sent_contents, doc_contents, states, valid_sentences,),
        )

        return list(map(list, zip(*mcts_idxs)))
    ...
    def forward(self, batch, subset):
        ...
        if subset == "train":
            ...
            mcts_idxs = self.mcts_inference(
                sent_contents, doc_contents, states, valid_sentences
            )

            for step_idxs in mcts_idxs:
                states, mcts_rewards = self.environment.update(step_idxs)
            ...
            return mcts_rewards.mean(0), greedy_rewards.mean(0)
    ...
    def validation_step(self, batch, batch_idx):
        mcts_rewards, greedy_rewards = self.forward(batch, subset="val")
        ...
        reward_dict = {
            ...
            "val_mcts_rouge_1": mcts_rewards[0],
            "val_mcts_rouge_2": mcts_rewards[1],
            "val_mcts_rouge_L": mcts_rewards[2],
            "val_mcts_rouge_mean": mcts_rewards.mean(),
        }
    ...
    def validation_epoch_end(self, outputs):
        ...
        self.lr_scheduler.step(output_dict["log"]["val_mcts_rouge_mean"])
    ...
    def test_step(self, batch, batch_idx):
        mcts_rewards, greedy_rewards = self.forward(batch, subset="test")
        ...
        reward_dict = {
            ...
            "test_mcts_rouge_1": mcts_rewards[0],
            "test_mcts_rouge_2": mcts_rewards[1],
            "test_mcts_rouge_L": mcts_rewards[2],
            "test_mcts_rouge_mean": mcts_rewards.mean(),
        }
    ...
    def from_config(dataset, reward, config):
        return RLSumValueInference(dataset, reward, config,)
...
class RLSummModel(torch.nn.Module):
    ...
    def produce_affinities(self, sent_contents, doc_contents, states, valid_sentences):
        ...
        probs[probs > 0] = (10 * probs[probs > 0]).exp()
