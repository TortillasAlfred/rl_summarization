...
class Analyzer:
    def analyze_dataset_embeddings_pair(self, dataset, embeddings_path):
        ...
        texts_analysis = list(
            map(
                self.analyze_text,
                datetime_tqdm(dataset, desc="Analyzing dataset texts..."),
            )
        )
    ...
    def article_stats_section(self, texts_analysis):
        ...
        section += self._per_article_stat(
            [t["n_tokens_content"] for t in texts_analysis],
            "number of tokens per content",
        )
        section += self._per_article_stat(
            [t["n_sents_content"] for t in texts_analysis],
        ...
        section += self._per_article_stat(
            [t["n_tokens_abstract"] for t in texts_analysis],
            "number of tokens per abstract",
        )
        section += self._per_article_stat(
            [t["n_sents_abstract"] for t in texts_analysis],
    ...
    def analyze_text(self, text):
        ...
        # Unique tokens
        content_unique_tokens = set([word for sent in split_content for word in sent])
        abstract_unique_tokens = set([word for sent in split_abstract for word in sent])
