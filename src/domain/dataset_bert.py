...
class CnnDailyMailDatasetBert():
    ...
    def tokenized_dataset(self, dataset):
        ...
        return {"train": (dataset["train"]["id"], train_articles, train_abstracts, dataset["train"]["article"], dataset["train"]["abstract"]),
                "test": (dataset["train"]["id"], test_articles, test_abstracts, dataset["test"]["article"], dataset["test"]["abstract"]),
                "val": (dataset["val"]["id"], val_articles, val_abstracts, dataset["val"]["article"], dataset["val"]["abstract"])}
...
class DatasetBertWrapper(CnnDailyMailDatasetBert):
    ...
    def gen_subsets(self):
        ...
        for subset_, document_tuples in self.dataset.items():
            examples=[]
            for properties in zip(document_tuples[0], document_tuples[1], document_tuples[2], document_tuples[3], document_tuples[4]):
