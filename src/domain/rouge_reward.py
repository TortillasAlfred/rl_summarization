...
def multi_rouge_n(sequences, scores_ids, n=2, exclusive=True):
    ...
    """
    ngrams = [
        _get_ngrams(n, sequence, exclusive=exclusive) for sequence in sequences
    ]
    ...
    for hyp_id, ref_id in scores_ids:
        ...
        scores += [
            f_r_p_rouge_n(evaluated_count, reference_count, overlapping_count)
        ]
    return scores
...
def rouge_n(evaluated_sentences,
    ...
    evaluated_ngrams = _get_ngrams(n, evaluated_sentences, exclusive=exclusive)
    reference_ngrams = _get_ngrams(n, reference_sentences, exclusive=exclusive)
    ...
    if raw_results:
        ...
        return f_r_p_rouge_n(evaluated_count, reference_count,
                             overlapping_count)
...
if __name__ == '__main__':
    ...
    train_loader, val_loader, test_loader = dataset.get_loaders(batch_size=5)
